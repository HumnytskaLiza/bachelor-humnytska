...
export function AssignedUsersTableSkeleton() {
  return (
    <div className="h-full">
      <ul role="list" className="divide-y divide-gray-100">
        {Array.from({ length: 3 }).map((_, i) => (
          <li key={i} className="flex justify-between gap-6 items-center px-4">
      ...
      {Array.from({ length: 3 }).map((_, i) => (
        ...
        >
          ...
          <div className="flex flex-col gap-2.5 w-full">
            <hr className="border-5 border-gray-200 rounded-2xl w-full" />
            <hr className="border-5 border-gray-200 rounded-2xl w-[80%]" />
          </div>
        </li>
      ))}
    </ul>
